...
<head>
    ...
    <title>Edit Question #{{ $question->id }} | Admin | The Entry Pass</title>
    ...
    <main class="admin-wrapper">
        <div class="container" style="max-width: 72rem;">
            <div class="admin-header">
                <h1 class="admin-title">Edit Question #{{ $question->id }}</h1>
            ...
            <div class="admin-card">
                <div class="question-preview">
                    <div class="question-preview-label">Current Question</div>
                ...
                <form method="POST" action="{{ route('admin.questions.update', $question) }}">
                        ...
                        <div class="form-field">
                            ...
                            <select name="correct_answer" required>
                                ...
                                @foreach(['A','B','C','D','E','F','G'] as $opt)
                                    <option value="{{ $opt }}" {{ old('correct_answer', $question->correct_answer) === $opt ? 'selected' : '' }}>{{ $opt }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-field">
                        ...
                        <textarea name="question" required placeholder="Enter your question here...">{{ old('question', $question->question) }}</textarea>
                    </div>

                    <div class="form-field">
                        <label>Extract / Context (Optional)</label>
                        <textarea name="extract" placeholder="Add context, passage, or scenario for this question...">{{ old('extract', $question->extract) }}</textarea>
                    ...
                    <div class="form-grid">
                        @foreach(['A','B','C','D','E','F','G'] as $letter)
                            <div class="form-field">
                                ...
                                <textarea name="option{{ $letter }}" placeholder="Option {{ $letter }} text...">{{ old('option'.$letter, $question->{'option'.$letter}) }}</textarea>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="form-field">
                        ...
                        <textarea name="rationale" placeholder="Explain why the correct answer is right...">{{ old('rationale', $question->rationale) }}</textarea>
                        <p class="hint-text">Helps students learn from their mistakes</p>
                    </div>

                    <div class="form-field">
                        <label>Study Tip (Optional)</label>
                        <textarea name="study_tip" placeholder="Add a helpful hint or memory aid...">{{ old('study_tip', $question->study_tip) }}</textarea>
                    </div>
